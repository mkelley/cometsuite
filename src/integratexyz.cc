/***************************************************************************
  ...
  The integratexyz main program.

  Copyright (C) 2010 by Michael S. Kelley
  <msk@astro.umd.edu>
...
#define SUBPROJECT "integratexyz"

using namespace std;

bool parseCommandLine(int, char**, vector<string>&, string&, double&, double&);
void usage();
...
int main(int argc, char *argv[])
{
  ...
  string outfile;
  double jd = -1;
  double time_d = 0;  // days
  double time_s = 0;  // seconds
  long max = 0;
  long n = 0;
  int status;

  if (parseCommandLine(argc, argv, xyzfileNames, outfile, time_d, jd)) {
  ...
  for (int i=0; i<xyzfileNames.size(); i++) {
	...
	jd = outParameters.obsDate() + time_d;
	outParameters.obsDate(jd);
      }
    ...
    time_s = (jd - inParameters.obsDate()) * 86400;
    for (n=0; n<max; n++) {
...
bool parseCommandLine(int argc, char** argv, vector<string>& xyzfileNames,
		      string& outfile, double& time, double& jd) {
